...
This repository is a personal job-market intelligence system.

It is not a scraper demo and not a collection of one-off job-search scripts. The
current architecture combines defensive ingestion, employer/source discovery,
evidence gates, agent-based planning, controlled connector activation, and
observability.
...
aggregators. Aggregators are treated as discovery inputs, not automatically trusted
source truth.
...
| `docs/architecture/` | Current architecture entry points and diagrams. |
...
1. finish README/runbook rebaseline,
2. continue ADR status review,
3. build a concise operator-oriented architecture narrative,
