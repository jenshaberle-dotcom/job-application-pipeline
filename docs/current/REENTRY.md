...
- canonical installed/released Product baseline: desktop **1.1.4**, source `de9da4dac585c23a39fa6006da6672021f38a228`;
...
- 1.1.5 also separates **All jobs inventory** from **Current** truth in labels/counts, so the sidebar no longer renders `0` beside a populated inventory or calls the inventory filter `All current`;
...
The next Product gate is candidate **1.1.5**. In All jobs, selecting dead Accompio Silver #626 must trigger an exact live check, persist the explicit closure, refresh the Product payload and remove #626 from Current authority without requiring F6 to be opened first. The inventory badge/filter must remain truthful (`All jobs` vs `Current`). Then open a genuinely live Employer-Origin target and enter F6. Before any model request the UI must explicitly show bundled Codex version plus ChatGPT authentication state. If ChatGPT auth is absent, the only accepted blocker is `codex_auth_required` / `codex_chatgpt_auth_required`; if connected, **Generate review text** becomes available and the first real vacancy-specific CV + letter adaptation may proceed. A failed 1.1.5 updater attempt must not immediately re-prompt the same target.
