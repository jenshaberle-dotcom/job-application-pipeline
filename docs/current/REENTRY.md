...
## CURRENT ACTIVE CAMPAIGN — FREEZE II / LINKEDIN + INDEED SENSOR EXPANSION FIRST

This section supersedes older F5/updater sequencing statements retained below as historical incident evidence.

- **Freeze II — Source Truth & Connector Reliability is ACTIVE.** Canonical issue: #1038. Canonical plan: `docs/planning/active/FREEZE-II-SOURCE-TRUTH-CONNECTOR-RELIABILITY.md`. Current sequence: `S0 COMPLETE -> LinkedIn/Indeed conservative market sensors -> sensor-driven candidate expansion -> freeze expanded candidate cohort -> generic connector generation (~95% strong target; 100% eventual smoke) -> systematic live search -> company/source understanding + metadata hardening -> optional ML Shadow pilot -> reassess`.
...
- **Freeze-II expands evidence before freezing the connector-fleet denominator.** S0 measured 67 initial candidates. LinkedIn and Indeed now enter as conservative discovery-only market sensors. They may use only an approved API/search-provider transport, never personal login/browser automation or broad scraping, and they emit only minimal discovery evidence. Their immediate purpose is to find genuinely new employer/source candidates. After known-candidate suppression and bounded promotion, the expanded candidate cohort becomes the S1 denominator.
- **Generic connector creation is the next engineering focus after candidate expansion.** Measure `generic_builder_success_rate` against the complete frozen expanded cohort. ~95% is a strong target, not a hard gate, and difficult candidates may not be removed from the denominator. Reusable Origin/Inventory/Detail/Proof classes are solved first; the long tail is then closed to 100% technical smoke coverage and one live-search outcome per candidate. Real jobs still require unchanged strict Employer-Origin proof.
...
- **Metadata/ML ordering:** broader connector/live-job evidence comes first. S3/S4 then harden title/company/location/work-model/employment/date/compensation/identifier/requirements provenance. Only if the broadened reviewed dataset is large enough may a first ML model train in Shadow on one bounded residual task; deterministic source truth remains authority.
