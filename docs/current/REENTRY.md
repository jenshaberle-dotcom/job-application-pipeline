...
- candidate **1.1.17** keeps fail-closed exact-detail verification but makes title identity presentation-neutral: connector/gender noise is removed, HTML/JSON title-bearing surfaces are matched by normalized token sequence, and a strongly matching exact-detail URL slug can confirm titles with at least three meaningful tokens. A generic careers/listing page body does not gain authority merely because it happens to list the job somewhere;
