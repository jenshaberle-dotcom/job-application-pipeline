...
## CURRENT ACTIVE CAMPAIGN — FREEZE II SOURCE TRUTH / S0 COMPLETE / S1-S2 ACTIVE
...
- **S1/S2 residual classification is the sole next action before mutation.** Reuse the existing Origin-plan, Inventory surface/bridge, Detail-surface and canonical generic Product proof paths to classify the current residual population by reusable technical/source family. Source activation, connector materialization and Product writes remain zero until this classification selects the first high-lift cohort.
