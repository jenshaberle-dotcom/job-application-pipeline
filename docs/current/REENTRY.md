...
## CURRENT ACTIVE CAMPAIGN — FREEZE II / S1 CONNECTOR FLEET COMPLETENESS PRIMARY
...
- **Freeze-II primary target is the complete current connector fleet, not a small high-lift subset.** The S0 baseline contains 67 connector candidates. S1 targets 67/67 materialized connector instances/configurations with a deterministic non-persistable synthetic smoke job, followed by one systematic live-source search outcome for every baseline candidate. A live zero-yield is valid when the source was actually searched; an unimplemented/unsearched candidate is not. Synthetic smoke evidence never enters Bronze/Silver/Product.
- **S1/S2 residual classification orders implementation waves; it does not cap scope.** Reusable Origin/Inventory/Detail/Proof classes should be solved first for leverage, then the remaining long tail must be closed until the 67-candidate baseline fleet reaches 100% connector-smoke completeness and every candidate has a live-search result. Real jobs still require unchanged strict Employer-Origin proof before ingestion.
