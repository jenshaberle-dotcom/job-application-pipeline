...
- canonical installed/released Product baseline: desktop **1.0.79**, source `0fe761a0c56abd7388a5b8273690511235155f3e`;
...
- current candidate is **1.0.80**: initial assessment binds `job_product_assessments.policy_version` to the job-evidence/hard-filter policy and records the independent ranking-policy version only as evidence metadata; exact preflight/apply revalidates both versions separately;
...
The 1.0.79 Product gate is accepted. The next engineering gate is exact-head 1.0.80 qualification/release of the policy-version decoupling above. After that, the only remaining authority-changing step is the already-preflighted **single initial assessment insert for Silver #626**. It must not run without explicit operator approval. The frozen preflight fingerprint is `78d9d24f1ecfc25488c47c77e0585d54a62c0b2729b6fb417dd403be57333e19`; any candidate, policy, observation or fingerprint drift invalidates the approval and requires a new read-only plan. The insert may establish Origin validation/activity from already-admitted employer-origin evidence, but hard-filter state, capability fit, ranking, Top-5, application and submission authority remain untouched.
