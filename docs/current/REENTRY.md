...
The next Product gate is candidate **1.1.5**. In All jobs, selecting dead Accompio Silver #626 must trigger an exact live check, persist the explicit closure, refresh the Product payload and remove #626 from Current authority without requiring F6 to be opened first. The All-jobs badge and visible list must remain the same canonical current Employer-Origin cohort. Then open a genuinely live Employer-Origin target and enter F6. Before any model request the UI must explicitly show bundled Codex version plus ChatGPT authentication state. If ChatGPT auth is absent, the only accepted blocker is `codex_auth_required` / `codex_chatgpt_auth_required`; if connected, **Generate review text** becomes available and the first real vacancy-specific CV + letter adaptation may proceed. A failed 1.1.5 updater attempt must not immediately re-prompt the same target.
