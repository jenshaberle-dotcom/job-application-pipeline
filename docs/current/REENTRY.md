...
- canonical installed/released Product baseline: desktop **1.1.10**, source `68968f425ebb08f275dfe15c41bd0ed85eacd38e`;
...
The next Product gate is candidate **1.1.11** on live **Eraneos Silver #511 — Data Engineer (all genders)**, benchmarked against the accepted Hornet application. In Quality-AI mode, audit must prove `gpt-5.6-sol / high` and current CV + current letter + exact vacancy shared under the split authority contract. The resulting letter must be complete, vacancy-specific and free of malformed/truncated text; the CV footer date must be current while signature/graphics remain unchanged. Final export must yield the verified 3-page PDF plus an editable DOCX companion. Separately prove Local-only mode makes zero LLM/provider requests and that the no-upload Application surface can download the fillable provider-free Word starter.
