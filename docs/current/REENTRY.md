...
- current candidate **1.1.1** therefore ships one checksum-pinned official Codex CLI `0.154.0` Linux x64 binary inside the immutable JAP runtime bundle, validates its identity before startup/cutover, and exposes it to F6 through `JAP_CODEX_EXECUTABLE`; installed runtime performs no Codex download/install/update;
...
The 1.0.80 delivery and the guarded Silver #626 initial-assessment mutation are complete. The next Product gate is installed **1.1.0**. Re-open accompio Silver #626 through **Prepare application** and require the same target to show a generation-ready context with Origin validated, hard filter still unknown, 2 grounded Candidate Fact claims and 2/2 exact F6 templates. **Generate review text** must now be enabled. Trigger it once and review the produced text; no application/submission/send authority is granted. If that operator-visible draft is correct, the following gate is the first real **Create finished application PDF** render under the frozen source-manifest and pixel-identity contract.
