...
## F. Top-5, Affinity and ranking semantics
...
| `PD-051` | approved | Minimum overall quality is 70/100. Fewer than five jobs are valid when fewer jobs qualify. The threshold is an adjustable V1 starting value, not a permanent constant. Reaffirmed 2026-09-16 after F4B proved the runtime 60 value was historical DEMO-001 drift rather than a superseding Product Decision. |
| `PD-052` | approved | The deterministic score components and weights are the initial **Affinity/desirability** authority: profile/ML direction 40%, Reliability potential 25%, Data/Data-Engineering focus 20%, origin/evidence quality 15%. Affinity answers only `Will ich diesen Job?` and must be calculable independently of Candidate Fit/hard-filter completion. Jobs within 3 Affinity points are otherwise comparable; only inside that window may hybrid, commute and public-transport preferences reorder them. Affinity may never override a hard Fit conflict or missing required Fit evidence. Reinterpreted/confirmed as F4B Affinity authority 2026-09-16; weights remain versioned and adjustable. |
...
| `PD-059` | approved | F4B separates Affinity/desirability (`Will ich diesen Job?`) from evidence-backed Candidate<->Job Fit (`Passt dieser Job zu mir?`), but the **current freeze campaign does not authorize numeric Fit automation or a Combined score**. The operator accepted the current truth that zero jobs have an authoritative Combined score. Capability auto-fit, generic skill-extraction hardening and Combined-score calibration are deferred as `F4B-FOLLOWUP-001` to the next freeze campaign, after broader connector maturity and a larger real-job cohort provide a stronger generic skill-evidence basis and/or enough evidence to qualify an ML-assisted layer. Missing Fit remains unknown; a hard Fit conflict cannot be averaged away by Affinity. Top 5 remains an `at_most_no_fill` projection over jobs that satisfy the existing currentness, Origin, hard-gate and evidence requirements. Approved 2026-09-16. |
