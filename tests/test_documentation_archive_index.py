...
def test_archive_index_collects_planning_items(tmp_path: Path) -> None:
    _write(tmp_path / "docs/planning/detail001.md", "# Detail Discovery\n")

    items = collect_archive_items(tmp_path, [Path("docs/planning")])

    assert len(items) == 1
    assert items[0].path == "docs/planning/detail001.md"
    assert items[0].suggested_bucket == "evidence_gate_history"


def test_archive_index_writes_index_files(tmp_path: Path) -> None:
    _write(tmp_path / "docs/planning/gov001.md", "# GOV 001\n")
    _write(tmp_path / "docs/source_analysis/stepstone.md", "# StepStone\n")
    ...
    assert "docs/planning/gov001.md" in planning_index
