...
def _target_country(target_locale: str | None) -> str | None:
    parts = [
        part.lower()
        for part in re.split(r"[-_]", str(target_locale or ""))
        if part
    ]
...
def _stage_from_mapping(raw: Mapping[str, object]) -> RepairStage:
    return RepairStage(
        ...
        decision=(
            str(raw.get("decision")) if raw.get("decision") is not None else None
        ),
        selected_url=(
        ...
        blocker=(
            str(raw.get("blocker")) if raw.get("blocker") is not None else None
        ),
    )
...
def _scope_reason(
    *,
    ...
    if target_country and tld in COUNTRY_CODES and tld != target_country:
        return (
            f"selected origin country TLD {tld} conflicts with target country "
            f"{target_country}"
        )
    ...
    if (
        target_country
        and origin_agent.is_known_ats_provider_domain(hostname)
        and path_segments
    ):
...
def normalize_selection_scope_outcome(
    ...
    *,
    ...
    if not isinstance(repair, Mapping):
        return result
    selected_url = str(
        repair.get("selected_url") or payload.get("selected_url") or ""
    ).strip()
    if not selected_url:
        return result
    ...
    outcome = finalize_outcome(
        company_key=str(
            repair.get("company_key") or payload.get("company_key") or ""
        ),
        company_name=str(
            repair.get("company_name") or payload.get("company_name") or ""
        ),
...
__all__ = ["normalize_selection_scope_outcome"]
