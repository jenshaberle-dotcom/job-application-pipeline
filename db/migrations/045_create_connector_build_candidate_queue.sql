...
--
-- Purpose:
--   Translate the latest S7N connector feasibility evidence into a stable
--   Gold read model for build planning. This queue tells the product which
--   candidates should move toward connector build planning, URL repair,
--   sample-job review or renewed origin discovery.
--
-- Boundary:
--   Read-only views only. This migration does not build connector artifacts,
--   approve build requests, register connectors, activate sources, write
--   Bronze records or change scheduler configuration.
...
drop view if exists gold_connector_build_queue_summary;

create view gold_connector_build_queue_summary as
select
    now() as generated_at,
    count(*)::integer as queued_candidate_count,
    count(*) filter (where queue_action = 'build_candidate_recommended')::integer as build_candidate_recommended_count,
    count(*) filter (where queue_action = 'origin_url_repair_required')::integer as origin_url_repair_required_count,
    count(*) filter (where queue_action = 'sample_job_review_required')::integer as sample_job_review_required_count,
    count(*) filter (where queue_action = 'origin_source_discovery_required')::integer as origin_source_discovery_required_count,
    count(*) filter (where queue_action = 'continue_existing_build_flow')::integer as continue_existing_build_flow_count,
    count(*) filter (where queue_action = 'monitor_existing_source')::integer as monitor_existing_source_count,
    count(*) filter (where queue_action = 'manual_source_review_required')::integer as manual_source_review_required_count,
    count(*) filter (where queue_action = 'monitor_or_manual_review')::integer as monitor_or_manual_review_count,
    min(queue_priority) as highest_attention_priority,
    max(last_signal_at) as latest_signal_at
