...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate SENSOR-001D BA remote/nationwide bounded sample-run plan without running ingestion."
    )
    parser.add_argument("--dsn", default=None, help="Optional PostgreSQL DSN. Defaults to JOB_PIPELINE_DATABASE_URL / DATABASE_URL.")
    parser.add_argument("--source-name", default=BA_SOURCE_NAME, help="Source name to inspect. SENSOR-001D currently supports BA only.")
    parser.add_argument("--max-terms", type=int, default=2, help="Maximum number of review-profile terms to include in the sample plan.")
    parser.add_argument("--output-dir", default="exports", help="Directory for JSON/Markdown plan artifacts.")
...
def main() -> int:
    ...
    json_path = output_dir / f"sensor001d_ba_remote_bounded_sample_run_plan_{stamp}.json"
    md_path = output_dir / f"sensor001d_ba_remote_bounded_sample_run_plan_{stamp}.md"
    ...
    if args.source_name != BA_SOURCE_NAME:
        ...
        try:
            ...
            exit_code = 0 if report["overall_status"] == "sample_plan_ready" else 1
...
def build_database_unavailable_report(source_name: str, error: BaseException) -> dict[str, Any]:
    return {
        ...
        "sample_terms": [],
        "findings": [],
        "next_action": "Start the database, configure a DSN, or use the docker psql fallback, then rerun SENSOR-001D.",
        "safety_boundary": {
            "read_only_plan": True,
...
def build_unsupported_source_report(source_name: str) -> dict[str, Any]:
    return {
        ...
        "sample_terms": [],
        "findings": [],
        "next_action": "Create a dedicated bounded sample-run plan for this market sensor.",
        "safety_boundary": {
            "read_only_plan": True,
            "external_requests": False,
            "database_writes": False,
