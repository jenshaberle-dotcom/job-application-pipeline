...
class {class_name}(JobSourceConnector):
    # Bounded employer-origin connector candidate generated from DB-backed S2V gate evidence.
...
def connector_docs_content(candidate: SourceCandidate, spec: dict[str, Any]) -> str:
    ...
    class_name = class_name_for(candidate)
    detail_urls = extract_detail_urls_from_spec(spec)
    detail_section = "\n".join(f"- {url}" for url in detail_urls) if detail_urls else "- no detail URLs were available in connector-candidate evidence"

    return f"""# {candidate.company_name} Connector Candidate Implementation

## Status

Generated by the S2V employer-origin connector implementation agent.
...
def build_implementation(candidate: SourceCandidate, gate: dict[str, Any]) -> ConnectorImplementation:
    ...
    return ConnectorImplementation(
        ...
        docs_path=Path("docs/source_analysis") / f"{module_name}_connector_candidate_implementation.md",
...
def gate_stop_lines(candidate: SourceCandidate, reason: str) -> list[str]:
    return [
        ...
        f"STOP: {reason}",
        "No connector candidate files were written.",
    ]
...
def run_agent(args: argparse.Namespace) -> int:
    ...
    print("Connector candidate files written.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate a bounded employer-origin connector implementation candidate from DB-backed gate evidence."
    )
