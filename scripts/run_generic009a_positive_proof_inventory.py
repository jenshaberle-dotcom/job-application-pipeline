...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GENERIC-009A positive proof inventory.")
    parser.add_argument(
        "--limit",
        type=int,
        default=100,
        help="Maximum employer-origin candidate rows to inspect.",
    )
    parser.add_argument(
        "--export-dir",
        type=Path,
        default=Path("exports/generic009a_positive_proof_inventory"),
        help="Output directory for JSON/Markdown inventory artifacts.",
    )
...
def main() -> int:
    ...
    report = build_positive_proof_inventory(rows, source=f"employer_origin_source_candidates:{db_status}")
    outputs = write_outputs(report, args.export_dir)
    summary = report.get("summary", {})

    print("# GENERIC-009A Positive Proof Inventory")
    print(f"db_status={db_status}")
    print(f"overall_status={report.get('overall_status')}")
    for key in [
        "input_row_count",
        "recommended_positive_control_count",
        "recommended_positive_control_keys",
        "covered_gap_ids",
        "missing_gap_ids",
        "gap_candidate_counts",
    ]:
