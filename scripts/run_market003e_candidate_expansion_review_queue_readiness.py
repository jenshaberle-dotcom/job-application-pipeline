...
DEFAULT_INPUT = "exports/market003d_candidate_expansion_review_action_plan/market003d_candidate_expansion_review_action_plan.json"
DEFAULT_OUTPUT_DIR = "exports/market003e_candidate_expansion_review_ui_queue_readiness"


def main() -> int:
    parser = argparse.ArgumentParser(description="Build MARKET-003E candidate expansion review UI/queue readiness model.")
    ...
    if not input_path.exists():
        report = build_missing_input_report(input_path, generated_at=generated_at)
    else:
        try:
            ...
            report = build_invalid_input_report(input_path, warning, generated_at=generated_at)
...
def print_summary(report: dict, outputs: dict[str, str]) -> None:
    ...
    print("# MARKET-003E Candidate Expansion Review UI/Queue Readiness")
    print(f"input_status={report.get('input_status')}")
    if report.get("input_warning"):
        print(f"input_warning={report.get('input_warning')}")
    print(f"card_count={summary.get('card_count', 0)}")
    print(f"review_required_count={summary.get('review_required_count', 0)}")
